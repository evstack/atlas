...
struct SolcOutput {
    contracts:
        Option<std::collections::HashMap<String, std::collections::HashMap<String, SolcContract>>>,
...
pub async fn verify_contract(
    ...
    let existing: Option<(String,)> =
        sqlx::query_as("SELECT address FROM contract_abis WHERE LOWER(address) = LOWER($1)")
            .bind(&address)
    ...
    if !bytecodes_match(
        ...
        return Err(AtlasError::BytecodeMismatch(
            "Compiled bytecode does not match deployed bytecode. Check compiler settings."
                .to_string(),
        )
        .into());
    }
    ...
    let constructor_args_bytes = request
        .constructor_args
        .as_ref()
    ...
    let source_files: Option<serde_json::Value> = if is_multi_file {
        ...
        serde_json::from_str(&request.source_code)
            .ok()
...
pub async fn get_contract_abi(
    ...
    match contract {
        Some((addr, abi, contract_name, verified_at)) => Ok(Json(ContractAbiResponse {
            address: addr,
            abi,
            contract_name,
            verified_at,
...
async fn compile_contract(
    ...
    let mut child = cmd.spawn().map_err(|e| {
        AtlasError::Compilation(format!(
            "Failed to spawn solc: {}. Is solc installed at '{}'?",
            e, solc_to_use
        ))
    })?;
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        stdin.write_all(input.as_bytes()).await.map_err(|e| {
            AtlasError::Compilation(format!("Failed to write to solc stdin: {}", e))
        })?;
    }
    ...
    let contract = found_contract.ok_or_else(|| {
        AtlasError::Compilation(format!(
